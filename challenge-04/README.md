...
carro.adicionarPessoas = function(pessoas) {
  var qtdeDisponivel = carro.assentos - carro.quantidadePessoas;

  if (carro.quantidadePessoas >= carro.assentos) {
    return 'O carro já está lotado!';
  } else if (carro.quantidadePessoas + pessoas > carro.assentos) {
    return `Só cabem mais ${qtdeDisponivel} ${qtdeDisponivel === 1 ? 'pessoa' : 'pessoas'}!`;
  } else {
...
carro.obterMarcaModelo(); // 'Esse carro é um  undefined'
