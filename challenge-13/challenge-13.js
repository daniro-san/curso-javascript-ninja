(function () {
  ...
  */
  var newBrasil = [];
  brasil.forEach(function (el, i) {
    newBrasil.push({ id: i, estado: el });
  });
  ...
  */
  ...
  var sevenLetters = brasil.every(function (el) {
    return el.length > 7;
  });
  ...
  */
  ...
  var hasCeara = brasil.some(function (el) {
    return el === "Ceará";
  });
  ...
  */
  var map = newBrasil.map(function (el, i) {
    return { id: el.id + 1, estado: `${el.estado} pertence ao Brasil.` };
  });
  ...
  */
  var filter = map.filter(function (el, i) {
    return el.id % 2 === 0;
  });
